...
nps_im1 <- st_read(paste0(path, "GIS/NPS_units_in_CGI_20250827_WGS84.shp"))
...
net_count <- park_prop |> filter(IM_veg_mon == 1) |> select(UNIT_CODE, NETCODE, IM_veg_mon) |>
  group_by(NETCODE) |> summarize(num_parks = sum(!is.na(UNIT_CODE)))
